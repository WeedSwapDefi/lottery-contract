...
contract Lottery is LotteryOwnable, Initializable {
    ...
    constructor( IERC20 _weedSwap,
        LotteryNFT _lottery,
        uint256 _minPrice,
        uint8 _maxNumber,
        address _owner,
        address _adminAddress) public {

        initialize(_weedSwap, _lottery, _minPrice, _maxNumber, _owner, _adminAddress);
    }
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        weedSwap.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        weedSwap.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            weedSwap.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            weedSwap.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        weedSwap.safeTransfer(address(msg.sender), _amount);
